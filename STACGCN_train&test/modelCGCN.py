...
class CGCN(nn.Module):
    ...
    def forward(self, idx, 
        ...
        for ii in range(batch_num):
            exemplar        = x0_1STA[ii, :, :, :][None].contiguous().clone()
            query           = x1_1STA[ii, :, :, :][None].contiguous().clone()
            query1          = x2_1STA[ii, :, :, :][None].contiguous().clone()
            for passing_round in range(self.propagate_layers):
                attention0  = self.extra_conv_fusion(torch.cat([self.generate_attention(exemplar, query),  self.generate_attention(exemplar, query1)],1)) 
                attention1  = self.extra_conv_fusion(torch.cat([self.generate_attention(query, exemplar),  self.generate_attention(query, query1)],1))
                attention2  = self.extra_conv_fusion(torch.cat([self.generate_attention(query1, exemplar), self.generate_attention(query1, query)],1))
                
                h_v0        = self.extra_ConvGRU(attention0, exemplar)
                h_v1        = self.extra_ConvGRU(attention1, query)
                h_v2        = self.extra_ConvGRU(attention2, query1)
                ...
                if passing_round == self.propagate_layers - 1:
                    ...
                    self.map_sal0[ii]   = self.my_fcn(h_v0, x0_1[ii, :, :, :][None].contiguous(), input_size)  # [1, 256, 45, 45]
                    ...
                    self.map_sal1[ii]   = self.my_fcn(h_v1, x1_1[ii, :, :, :][None].contiguous(), input_size)  # [1, 256, 45, 45]
                    ...
                    self.map_sal2[ii]   = self.my_fcn(h_v2, x2_1[ii, :, :, :][None].contiguous(), input_size)  # [1, 256, 45, 45]
