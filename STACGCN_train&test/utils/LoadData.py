...
def test_data_loader(args, test_path=False, segmentation=False):
    ...
    tsfm_grount = transforms.Compose(
        [transforms.Resize(input_size), transforms.ToTensor()])

    img_test = VOCDatasetE(audioroot_dir=args.audio_dir2, swtichroot_dir=args.swtich_dir2, imgroot_dir=args.img_dir2,
                           groundt_dir=args.groundt_dir2, num_classes=args.num_classes, transform_im=tsfm_test, transform_gt=tsfm_grount, test=True)
    val_loader = DataLoader(img_test,  batch_size=64,
...
class VOCDatasetT(Dataset):
    def __init__(self, audioroot_dir, swtichroot_dir, imgroot_dir, groundt_dir, num_classes=28, transform_im=None, transform_gt=None, test=False):
        ...
        self.image_list, self.name_list, self.figs_list, self.seqs_list = self.read_labeled_image_list_train(
            self.audioroot_dir, self.imgroot_dir, self.swtichroot_dir, self.groundt_dir)
        self.image_num = 2
        self.thres = 1000
    ...
    def __getitem__(self, idx):
        ...
        for hh in range(-4, 5):
            pick_path = os.path.join(img_la[5], img_la[7], img_la[8], '%04d' % (int(img_la[9][:-4])+hh)+'.jpg')
            image_pick = Image.open(pick_path).convert('RGB')
            image_pick = self.transform_im(
                image_pick.resize((256, 256), Image.ANTIALIAS))
                ...
                if os.path.exists(os.path.join(groundt_dir, ori_name[file], ficname[fs])):
                    for picp in range(3, len(picname)-3):
                        if picname[picp].endswith('_c.jpg'):
                            if os.path.exists(os.path.join(groundt_dir, ori_name[file], ficname[fs], '%04d' % (int(picname[picp][:-6]))+'_c.jpg')) | os.path.exists(os.path.join(groundt_dir, ori_name[file], ficname[fs], '%04d' % (int(picname[picp][:-6])-1)+'_c.jpg')) | os.path.exists(os.path.join(groundt_dir, ori_name[file], ficname[fs], '%04d' % (int(picname[picp][:-6])+1)+'_c.jpg')):
                                ...
                                ficnames = ficname[fs]
                                path_list.append(pv+'+'+pa+'+'+ps+'+'+str(file)+'+'+audio_dir+'+'+data_dir+'+' +
                                                 groundt_dir+'+'+ori_name[file]+'+'+ficname[fs]+'+'+picname[picp][:-6]+'.jpg')
...
class VOCDatasetE(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        for hh in range(-4, 5):
            pick_path = os.path.join(img_la[4], img_la[5], img_la[6], 'img_%05d' % (
                int(img_la[7][4:-4])+hh)+'.jpg')
            image_pick = Image.open(pick_path).convert('RGB')
            image_pick = self.transform_im(
                image_pick.resize((256, 256), Image.ANTIALIAS))
            images.append(image_pick)
            ...
            if hh in [-3, 0, 3]:
                image_pick2 = Image.open(pick_path).convert('RGB')
                image_pick2 = self.transform_im(
                    image_pick2.resize((356, 356), Image.ANTIALIAS))
                images2.append(image_pick2)
                Apick_path = os.path.join(img_la[3], img_la[5], img_la[6], 'img_%05d' % (
                    int(img_la[7][4:-4])+hh)+'_asp.h5')
                with h5py.File(Apick_path, 'r') as hf:
                    audio_features = np.float32(hf['dataset'][:])  # 5,128
                audio_pick = torch.from_numpy(audio_features).float()
                audios.append(audio_pick)
    ...
    def read_labeled_image_list_test(self, audio_dir, data_dir, swtich_dir, groundt_dir):
        ...
        for file in range(0, 1):  # len(ori_name)):
            ...
            for fs in range(0, len(ficname)):
                ...
                picname = xmmhh
                for picp in range(6, len(picname)-6, 30):
                    if picname[picp].endswith('.jpg'):
                        ...
                        pv = os.path.join(
                            data_dir, ori_name[file], ficname[fs], picname[picp])
                        pa = os.path.join(
                            audio_dir, ori_name[file], ficname[fs], picname[picp][:-4]+'_asp.h5')
                        picnames.append(picname[picp])
                        ficnames = ficname[fs]
                        path_list.append(pv+'+'+pa+'+'+str(file)+'+'+audio_dir+'+'+data_dir +
